...
Db::startTrans();
try {
	$info = Db::name("ybmp_bargain_order")->where(["out_trade_no" => $out_trade_no, "order_status" => 0])->find();
	$res = Db::name("ybmp_bargain_order")->where(["out_trade_no" => $out_trade_no, "order_status" => 0])->update($data);
	Db::name("ybmp_bargain_order_payment")->where(["out_trade_no" => $out_trade_no, "pay_status" => 0])->update($data_pay);
	Db::commit();
	Log::write($out_trade_no . ",支付状态更改成功", "bargain_pay_state_change_success");
	$ser = new OffwebService($info["mch_id"]);
	$r = $ser->sub_send($info["buyer_id"], "砍价订单支付成功:" . $info["order_no"], "kj_order_pay");
} catch (\Exception $e) {
	Db::rollback();
