...
class WxPayConfig
{
	 ...
	 * @var path
	 */
	//const SSLCERT_PATH =SITE_ROOT."addons/yb_mingpian/core/30_wechat_refund_cert.pem";
	//const SSLKEY_PATH = SITE_ROOT."addons/yb_mingpian/core/30_wechat_refund_key.pem";
    const SSLCERT_PATH ="";
    const SSLKEY_PATH = "";
	//=======【curl代理设置】===================================
	/**
	...
	function __construct(){
		...
		if(!empty($mch_id))
		{
			$param2=Db::name('account_wxapp')->where('uniacid',$mch_id)->field('key,secret,name')->find();
			$this->APP_ID=$param2['key'];
			$this->APPSECRET =  $param2['secret'];
			$param = Db::name('ybmp_config')->where("key","WXPAY")->where("mch_id",$mch_id)->value("value");
...
//			$this->APPSECRET = $param['APP_SECRET'];
			...
			if(!empty($GLOBALS['key']) && $GLOBALS['key'] == 'bargain'){
			...
			}elseif(!empty($GLOBALS['key']) && $GLOBALS['key'] == 'miaosha'){
				...
				$ms_url ="https://" . $host.$child_path[0] .'addons/yb_mingpian/core/ms_pay.php';
				$this->APP_URL = $param['APP_URL_MS']?$param['APP_URL_MS']:$ms_url;
			}else{
