...
class Upload extends Controller
{
	...
	public function uploadFile($mch_id)
	{
		...
		if ($ok["code"]) {
				...
				if ($old_path == "video/video/") {
					...
					if ($image_size) {
						...
						if ($old_path == "goods/") {
							...
							$retval = $this->photoCreate($this->file_path, $_W["siteroot"] . "addons/yb_mingpian/core/" . $ok["path"], "." . $file_name_explode[$suffix], $type, $pic_name, $album_id, $width, $height, $pic_tag, $pic_id, $ok["domain"], $ok["bucket"]);
							if ($retval > 0) {
			...
			$this->return["data"] = str_replace("http:/", "https:/", $this->return["data"]);
		} else {
			$this->return["message"] = "请检查您的上传参数配置或上传的文件是否有误";
		}
		return $this->ajaxFileReturn();
		...
		if ($this->upload_type == 1) {
			...
			foreach ($photoArray as $v) {
				if (stristr($type, $v["type"])) {
					$image->thumb($v["width"], $v["height"], \think\Image::mHEjL)->save($v["path"]);
				}
			}
		}
		$album = new Images();
		if ($pic_id == '') {
			$retval = $album->addPicture($album_id, $pic_name, $pic_tag, $_W["siteroot"] . "addons/yb_mingpian/core/" . $photoPath, $width . "," . $height, $width . "," . $height, $photoArray["bigPath"]["path"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["middlePath"]["path"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["smallPath"]["path"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["littlePath"]["path"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $this->upload_type, $domain, $bucket);
		} else {
			$retval = $album->ModifyAlbumPicture($pic_id, $_W["siteroot"] . "addons/yb_mingpian/core/" . $photoPath, $width . "," . $height, $width . "," . $height, $photoArray["bigPath"]["path"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["bigPath"]["width"] . "," . $photoArray["bigPath"]["height"], $photoArray["middlePath"]["path"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["middlePath"]["width"] . "," . $photoArray["middlePath"]["height"], $photoArray["smallPath"]["path"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["smallPath"]["width"] . "," . $photoArray["smallPath"]["height"], $photoArray["littlePath"]["path"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $photoArray["littlePath"]["width"] . "," . $photoArray["littlePath"]["height"], $this->instance_id, $this->upload_type, $domain, $bucket);
			$retval = $pic_id;
		}
		return $retval;
	}
	...
	public function photoAlbumUpload()
	{
		...
		if (!empty($_W["setting"]["remote"]["type"])) {
			...
			if (is_error($remotestatus)) {
				...
				$res = Db::name("ybmp_images")->insertGetId(["group_id" => $album_id, "img_name" => $newfile, "img_tag" => '', "img_cover" => $remoteurl]);
				$data["state"] = 1;
				$data["file_id"] = $res;
				$data["file_name"] = '';
				return $data;
		...
		if ($ok["code"]) {
			...
			if ($image_size) {
				...
				$pic_id = request()->post("img_id", '');
				$upload_flag = request()->post("upload_flag", '');
				$retval = $this->photoCreate($this->file_path, $ok["path"], "." . $file_name_explode[$suffix], $type, $pic_name, $album_id, $width, $height, $pic_tag, $pic_id, $ok["domain"], $ok["bucket"]);
				if ($retval > 0) {
					$album = new Images();
					...
					$data["origin_file_name"] = $this->file_name;
					$data["file_path"] = $_W["siteroot"] . "addons/yb_mingpian/core/" . $this->file_path . $newfile;
					$data["state"] = "1";
					return $data;
				} else {
					$data["state"] = "0";
