...
class Umupload extends Controller
{
	...
	public function uploadFile()
	{
		...
		$this->file_name = $_FILES["upfile"]["name"];
		$this->file_size = $_FILES["upfile"]["size"];
		$this->file_type = $_FILES["upfile"]["type"];
		if ($this->file_size == 0) {
		...
		$newfile = $guid . $ext;
		$ok = $this->moveUploadFile($_FILES["upfile"]["tmp_name"], $this->file_path . $newfile);
		...
		if (!empty($_W["setting"]["remote"]["type"])) {
			...
			if (is_error($remotestatus)) {
				...
				$this->return["url"] = $remoteurl;
				$this->return["title"] = $newfile;
		...
		if ($ok["code"]) {
			if (!strstr(UPLOAD_VIDEO, $this->file_path)) {
				...
				$this->return["state"] = "SUCCESS";
				$this->return["url"] = str_replace("http:/", "https:/", $_W["siteroot"] . "addons/yb_mingpian/core/" . $ok["path"]);
				$this->return["title"] = $newfile;
			} else {
	...
	public function photoAlbumUpload()
	{
		...
		if (!empty($_W["setting"]["remote"]["type"])) {
			...
			if (is_error($remotestatus)) {
				...
				$res = Db::name("ybmp_images")->insertGetId(["group_id" => $album_id, "img_name" => $newfile, "img_tag" => '', "img_cover" => $data["file_path"]]);
				$data["state"] = 1;
				$data["file_id"] = $res;
				$data["file_name"] = '';
				return $data;
