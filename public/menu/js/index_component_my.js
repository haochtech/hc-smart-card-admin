...
Vue.component('form_checkbox', {
    props: ['index','title','value','layout','list'],
    template: '<li  title="点击进行修改,拖动交换位置" class="ui-draggable"><div v-on:click="del_left(index)" class="deleteButton">' +
        '</div><div v-on:click="onc_banner(index,$event)"><div class="title">{{title}}</div>' +
        '<div :class="\'yb_checkbox yb_checkbox-\'+layout"  v-for="m in list">' +
        '<input style="display: block" type="checkbox" :value="m.value" class="es-input data-bind">{{m.value}}</div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            bannerVM.all_data.splice(index,1);
        ...
        onc_banner:function (index,e) {
            ...
            if (bannerVM.all_data[index]['layout']==1){
                $("#this_input_checkbox_layout1").prop("checked", "checked");
            }else {
                $("#this_input_checkbox_layout2").prop("checked", "checked");
            }
...
Vue.component('form_radio', {
    props: ['index','title','value','layout','list','time_key'],
    template: '<li title="点击进行修改,拖动交换位置" class="ui-draggable"><div v-on:click="del_left(index)" class="deleteButton"></div>' +
        '<div v-on:click="onc_banner(index,$event)"><div class="title">{{title}}</div>' +
        '<div :class="\'yb_radio yb_radio-\'+layout" v-for="m in list"><input style="display: block" type="radio" :value="m.value" :name="\'radio-\'+time_key"  :checked="m.checked" disabled="true" class="es-input data-bind">{{m.value}}</div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            bannerVM.all_data.splice(index,1);
        ...
        onc_banner:function (index,e) {
            ...
            if (bannerVM.all_data[index]['layout']==1){
                $("#this_input_radio_layout1").prop("checked", "checked");
            }else {
                $("#this_input_radio_layout2").prop("checked", "checked");
            }
...
Vue.component('form_pic', {
    props: ['index','title','time_key'],
    template: '<li  title="点击进行修改,拖动交换位置" class="ui-draggable"><div v-on:click="del_left(index)" class="deleteButton"></div><div v-on:click="onc_banner(index,$event)"><div class="title">{{title}}</div><div class="yb_pic"><img style="width: 100px;height: 100px;" src="'+host+'addons/yb_mingpian/core/public/menu/images/add_pic.jpg"></div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            bannerVM.all_data.splice(index,1);
...
Vue.component('form_pic_arr', {
    props: ['index','title','time_key'],
    template: '<li title="点击进行修改,拖动交换位置" class="ui-draggable"><div v-on:click="del_left(index)" class="deleteButton"></div><div v-on:click="onc_banner(index,$event)"><div class="title">{{title}}</div><div class="yb_pic_arr"><img style="width: 100px;height: 100px;" src="'+host+'addons/yb_mingpian/core/public/menu/images/add_pic.jpg"></div></div></li>',
    methods: {
        //删除
        del_left: function (index) {
            bannerVM.all_data.splice(index,1);
