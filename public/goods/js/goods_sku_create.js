...
var $goods_attribute = new Array();
var $goods_attribute_sku = new Array();
/**
 * 删除数组中的指定元素
 * @param arr
 * @param val
    ...
    $(".goods-sku .goods-sku-item span").live("click", function () {
        ...
        timeoutID = setTimeout(function () {
            ...
            if ($this.hasClass("selected")) {
                for (var i = 0; i < app.goods_sku.length; i++) {
                    if (spec_value_id == app.goods_sku[i].id) {
                        app.goods_sku.splice(i, 1);
                    }
                }
                ...
                var item = {};
                item['name'] = spec_value_name;
                item['id'] = spec_value_id;
...
function editSkuData(spec_obj_str, sku_data) {
    ...
    if ($sku_goods_picture.length > 0) {
        var default_spec_id = 0;
        var simglen = $sku_goods_picture.length;
        var default_spec_id1 = 0;
        for(var k in $goods_attribute)
        {
            var item = $goods_attribute[k];
            if(item.ismain)
            {
                default_spec_id = item.spec_id;
            }
            if(item.value.length === simglen && default_spec_id1 === 0)
            {
                default_spec_id1 = item.spec_id;
            }
        }
        if(default_spec_id === 0)
        {
            if(default_spec_id1 > 0)
            {
                default_spec_id = default_spec_id1;
            }
            else
            {
                default_spec_id = $sku_goods_picture[0]['attr_value_items'].split(":")[0];
            }
        }
        ...
        for (var m = 0; m < $specObj[i]["value"].length; m++) {
            ...
            if (selected_obj.data("spec-show-type") == 2) {
                ...
                if (src == "" || src == undefined) {
                    src = host + "addons/yb_mingpian/core/index.php?s=/public/admin/images/goods/goods_sku_add.png";
                }
...
function createSkuPictureBox(spec_id, spec_value_id, spec_name, spec_value_name) {
    ...
    var mainid = 0;
    var mainid1 = 0;
    for(var j in $goods_attribute)
    {
        var jitem = $goods_attribute[j];
        if(jitem['ismain'])
        {
            mainid = jitem['spec_id'];
        }
        if(jitem.value.length === $sku_goods_picture.length && mainid1 === 0)
        {
            mainid1 = jitem['spec_id'];
        }
    }
    if(mainid === 0 && mainid1 > 0)
    {
        mainid = mainid1;
    }
    if(mainid === spec_id)
    {
        for(var i in $sku_goods_picture)
        {
            var sitem = $sku_goods_picture[i];
            imgs[sitem.images] = sitem.sku_picture_query[0].img_cover;
        }
        try {
            for (var i = 0; i < $goods_attribute_sku.length; i++) {
                var sitem = $goods_attribute_sku[i];
                var keys = spec_id+":"+spec_value_id;
                if(sitem.attr_value_items.indexOf(keys) >= 0)
                {
                    sku_picture = imgs[sitem.images];
                    img_id = sitem.images;
                }
            }
        } catch (e) {
    ...
    if (sku_picture !== "") {
        ...
        html += '<img nstype="goods_image" src="' + sku_picture + '">';
        html += '<input type="hidden"  class="sku_upload_img_id" nstype="goods_image" spec_id="' + spec_id + '" spec_value_id="' + spec_value_id + '" value="' + img_id + '">';
        html += '<div class="black-bg" style="display: inline;" onclick="remlong(this);">';
        html += '<div class="sku-off-box">&times;</div>';
        html += '</div>';
        html += '</div>';
    } else {
        html += '<div class="upload-thumb" id="sku_default_uploadimg"> ';
        html += '<img src="' + host + 'addons/yb_mingpian/core/public/goods/img/default_goods_image_240.gif">';
        html += '</div>';
    }
    html += '</div>';
    html += '<div class="clear"></div>';
    html += '<div class="handle">';
    // html += '<div class="btn radius" style="background: #cccbcb;">';
    // html += '<a href="javascript:$(\'#spec_' + spec_value_id + '\').click();">';
    // html += '<span>';
    // html += '<input id="spec_' + spec_value_id + '" style="cursor:pointer;font-size:0;" file_type="sku" spec_id="' + spec_id + '" spec_value_id="' + spec_value_id + '" type="file"  hidefocus="true" class="input-file" name="file_upload" onclick="file_upload(this);" />';
    // html += '</span>';
    // html += '<p style="color:black;">图片上传</p>';
    // html += '</a>';
    // html += '</div>';
    //
    html += '<div class="btn radius" style="background: #cccbcb;color:black;" >';
    html += '<a id="sku_img_box" href="javascript:void(0);"spec_id="' + spec_id + '" spec_value_id="' + spec_value_id + '">从图片空间选择</a>';
    html += '</div>';
    //
    html += '</div>';
    html += '</div>';
